...
def saveinfoemploye():
        """
        Fonction qui exécute le script qui decoupe le grade de l'employe
        ...
        """
        print("saveinfoemploye() ...")
        sid = transaction.savepoint()
        try:
            employes = dao_employe.toListEmployesActifs()
            #on va parcourir les employes pour decouper leurs grade
            for item in employes:
                print(f"Employe {item}")
                grade_array = item.grade.split(".")
                newgrade = grade_array[0].strip()
                item.grade = newgrade.lower()
                item.save()

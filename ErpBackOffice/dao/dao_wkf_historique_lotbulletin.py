...
class dao_wkf_historique_lotbulletin(object):
    id = 0
    employe_id = None
    etape_id = None
    ...
    @staticmethod
    def toListHistoriqueOfLot(lot_id):
        return Model_Wkf_Historique.objects.filter(lot_bulletin_id = lot_id)

    @staticmethod
    def toCreateHistoriqueWorkflow(employe_id , etape_id , lot_bulletin_id = None):        
        try:
            historique_workflow = dao_wkf_historique_lotbulletin()
            historique_workflow.employe_id = employe_id
            historique_workflow.etape_id = etape_id
            historique_workflow.lot_bulletin_id = lot_bulletin_id
    ...
    def toSaveHistoriqueWorkflow(object_dao_wkf_historique):
        try:
            ...
            historique_workflow.lot_bulletin_id = object_dao_wkf_historique.lot_bulletin_id
    ...
    @staticmethod
    def getCountSignatures(etape_id, lot_bulletin_id):
        try:
            historique_workflow = Model_Wkf_Historique.objects.filter(etape_id = etape_id, lot_bulletin_id = lot_bulletin_id)            
    ...
    @staticmethod
    def toGetIfSigned(etape_id, lot_bulletin_id, employe_id):
        try:
            historique_workflow = Model_Wkf_Historique.objects.filter(etape_id = etape_id, lot_bulletin_id = lot_bulletin_id, employe_id = employe_id) 

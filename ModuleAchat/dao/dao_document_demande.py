...
class dao_document_demande(object):
    id = 0
    type_document = ""
    url_document = ""
    ...
    @staticmethod
    def toCreateDocument(type_document, url_document, description, demande_achat_id = None):
        try:
            document = dao_document_demande()
            document.type_document = type_document
            document.url_document = url_document
            document.demande_achat_id = demande_achat_id
    ...
    def toSaveDocument(auteur, objet_dao_document):
        try:
            ...
            document.demande_achat_id = objet_dao_document.demande_achat_id
    ...
    @staticmethod
    def toListDocumentbyDemande(id):
        return Model_Document.objects.filter(demande_achat_id = id)

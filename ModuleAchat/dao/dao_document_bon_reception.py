...
class dao_document_bon_reception(object):
    id = 0
    type_document = ""
    url_document = ""
    ...
    @staticmethod
    def toCreateDocument(type_document, url_document, description, bon_reception_id = None):
        try:
            document = dao_document_bon_reception()
            document.type_document = type_document
            document.url_document = url_document
            document.bon_reception_id = bon_reception_id
    ...
    def toSaveDocument(auteur, objet_dao_document):
        try:
            ...
            document.bon_reception_id = objet_dao_document.bon_reception_id
    ...
    def toListDocumentbyBonReception(id):
        return Model_Document.objects.filter(bon_reception_id = id)

...
class dao_taux_change(object):
    ...
    @staticmethod
    def toListTaux():
        return Model_Taux.objects.all().order_by("-date_creation")

    @staticmethod
    def toListTauxDuJour(date_jour):
        try:
            return Model_Taux.objects.filter(date_creation__date = date_jour.date()).order_by("-date_creation")
    ...
    def toListTauxCourant():
        try:
            return Model_Taux.objects.filter(est_courant = True).order_by("-date_creation")
        except Exception as e:
            #print("ERREUR")
            #print(e)
            return []
    ...
    def toSaveTaux(auteur, object_dao_taux):
        try:
            ...
            taux.date_creation = timezone.now()
            taux.save()
            return taux
        except Exception as e:

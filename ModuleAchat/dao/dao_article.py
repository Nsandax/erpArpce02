...
class dao_article(object):
    ...
    def toListArticlesOfServiceReferent(service_ref_id):
        ...
        return Model_Article.objects.filter(unite_fonctionnelle_id = service_ref_id).filter(est_achetable = True)
    ...
    def toListArticlesStockables():
        return Model_Article.objects.filter(est_stockable = True).order_by("designation")
    @staticmethod
    def toListArticlesStockablesByAuteur(user_id):
        return Model_Article.objects.filter(est_stockable = True).filter(auteur_id=user_id).order_by("designation")

    '''@staticmethod
    def toListArticlesStockablesOfServiceReferent(service_ref_id):
        return Model_Article.objects.filter(est_stockable = True).filter(unite_fonctionnelle_id = service_ref_id).order_by("designation")'''

    @staticmethod
    def toListArticlesStockablesOfServiceReferentByAuteur(service_ref_id, user_id):
        return Model_Article.objects.filter(est_stockable = True).filter(unite_fonctionnelle_id = service_ref_id).filter(auteur_id=user_id).order_by("designation")
    ...
    #@staticmethod
    #def toCountArticleByService():
    #    try:
    #        Liste_Service = Model_Unite_fonctionnelle.objects.all()
    #        nomber = 0
    #        ArtByService = {"designation":"","number":""}
    #        for unite in Liste_Service:
    #            nomber = Model_Article.objects.annotate(designation=unite.libelle)
    #            filter(unite_fonctionnelle_id = unite.id).count()
    #            ArtByService.update{"designation":}
    #    except Exception as e:

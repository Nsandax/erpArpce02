...
from ErpBackOffice.models import Model_Pret, Model_PaiementInterne, Model_DossierPaie
...
class dao_pret(object):
	...
	@staticmethod
	def toListPretAvailableOnDate(date_periode):
		list_pret = []
		prets =  Model_Pret.objects.filter(date_premiere_echeance__lte = date_periode).order_by('-id')
		for pret in prets:
			if pret.is_running and pret.date_cloture > date_periode:
				list_pret.append(pret)
		return list_pret
	...
	@staticmethod
	def toGetPretOfEmployeByPeriode(matricule, periode):
		try:
			month = periode.month
			year = periode.year
			pret = Model_Pret.objects.filter(date_premiere_echeance__month=month, date_premiere_echeance__year=year, employe_profilrh_matricule=matricule).first()
			return pret
			# pret.delete()
		except Exception as e:
			return None
	...
	def toComputeMontantARembourser(employe_id):
		try:
			...
			for pret in prets:
				...
				valeur += montant
				#print("toComputeMontantARembourser", pret, montant)
				if montant > 0:
					lignes = dao_ligne_paiement_pret.toListLignePaiementByPretAndDossierPaie(pret.id, dossier_paie.id).delete()
					ligne_paiement_pret = dao_ligne_paiement_pret.toCreateLignePaiementPret("Paiement pret {}".format(pret.reference), montant, pret.devise_id, pret.id, True, "Paiement pret", dossier_paie.id)

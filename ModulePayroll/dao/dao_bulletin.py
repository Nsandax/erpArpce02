...
class dao_bulletin(object):
    ...
    @staticmethod
    def toCreate(auteur_id, designation, employe_id, lot_id = None, reference = "", periode = "", type = ""):
        try:
    ...
    @staticmethod
    def toGetOfEmployeFromLot(employe_id, dossier_id):
        try:
            bulletin = Model_Bulletin.objects.filter(employe_id = employe_id, lot_id = dossier_id).first()
            return bulletin
        except Exception as e:
            #print("ERREUR LORS DU L'AFFICHAGE")
            #print(e)
    ...
    @staticmethod
    def toGetOfEmployeFromPeriode(employe_id, periode):
        try:
            mois = periode.month
            annee = periode.year
            #print("mois {} ::: annee {}".format(mois, annee))
            dossierpaie = Model_DossierPaie.objects.filter(mois = mois, annee=annee).first()
            #print('Dossier Paie', dossierpaie)
            dossierpaie_id = dossierpaie.id
            bulletin = Model_Bulletin.objects.filter(employe_id = employe_id, lot__dossier_paie__id = dossierpaie_id).first()
            #print('DANS BULLETIN', bulletin)
            return bulletin
        except Exception as e:
            print("ERREUR RECUPERATION BULLETIN IND")
            print(e)
            return None

...
def run():
    ...
    #SaveOtherRubrique("Nbre_Part_Trimeste_2")

@transaction.atomic
def showmereealdate(file_name):
        """
        Fonction qui exécute le script de test de date d'enregistrement
...
@transaction.atomic
def savenombrepart(file_name):
    """
    Fonction qui exécute le script d'Enregistrement de Nombre de Part
    :param file_name: (string) une chaine de caractère,le nom du fichier à importer
    :return : (void)  renvoie rien juste affiche le deroulement
    """
    print("savenombrepart() ...")
    sid = transaction.savepoint()
    try:
        import_dir = settings.MEDIA_ROOT
        file_dir = 'excel/'
        ...
        if default_storage.exists(file_path):
            ...
            df = pd.read_excel(io=filename, sheet_name=sheet)
            for i in df.index:
                print('-----Iteration Execution {}'.format(i))
                periodeStart = df["PERIOD_START_DATE"][i]
                periodeEnd = df["PERIOD_END_DATE"][i]
                payroll = df["PAYROLL_ID"][i]
                elementName = df["ELEMENT_NAME"][i]
                ReportName = df["REPORTING_NAME"][i]
                ClassName = df["CLASS_NAME"][i]
                EntryStartdate = df["ENTRY_START_DATE"][i]
                EntrEnddate = df["ENTRY_END_DATE"][i]
                inputvalName = df["INPUT_VALUE_NAME"][i]
                resultValue = df["RESULT_VALUE"][i]
                employe_num = df["EMP_NUMBER"][i]
                cl_ret = df["CL_RET"][i]

                #Enregistrement de Nombre de part
                NbrePart = Model_temp_Nbre_Part(periode_end_date=periodeStart,payroll_id=payroll,element_name=elementName,reporting_name=ReportName,
                class_name=ClassName, entry_start_date=EntryStartdate, entry_end_date=EntrEnddate,input_value_name=inputvalName,resultat_value=resultValue,
                employe_number=employe_num,cl_ret=cl_ret)
                NbrePart.save()

        transaction.savepoint_commit(sid)
    except Exception as e:
        print("---ERREUR SAVE NOMBRE PART---")
        print(e)
        transaction.savepoint_rollback(sid)
            ...
            if default_storage.exists(file_path):
                ...
                for i in df.index:
                    ...
                    if employe:
                        ...
                        elif reportname == 'Prime diplome' and inputval == 'Cycle':
                            print("item_bulletin Prime diplome")
                            rubprimediplome = dao_rubrique.toGetRubriqueByReference("PRIDIPLOM")
                            item_bulletin = dao_item_bulletin.toCreate(auteur.id, rubprimediplome.designation, rubprimediplome.id, bulletin.id, nombre, base, taux, resultval, rubprimediplome.sequence)
                            item_bulletin = dao_item_bulletin.toSave(item_bulletin)
                            print("item_bulletin PRIDIPLOM", item_bulletin)
            ...
            if default_storage.exists(file_path):
                ...
                for i in df.index:
                    ...
                    if employe:
                        ...
                        email = f"{first_name}.{last_name}@arpce.cg"
                        phone = ''
                        arrondissement = ""
                        adresse = ""
                        categorie = ""
                    ...
                    if employe:
                        ...
                        lot = dao_lot_bulletin.toGetOrCreatelotbyperiode(auteur.id,datepaiement,est_regulier)
                        lot_id = lot.id
                        ...
                        base = primebasecot
                        taux = 10.03
                        primeallfam = primebasecot *  base * (taux/100)
                        rubprimeallfam = dao_rubrique.toGetRubriqueByReference("ALLFAM")
                        ...
                        primetotgain = salbrut + rubindtrans + rubcmptrans + primeinddepart
                        ...
                        primetotret = cnss + irpp + rubpret + primeretavsal + rubcotmutuel
                        ...
                        if int(periode.month) == 1: primebaseconge = salbrut
                        elif int(periode.month) > 1: 
                            mois = periode.month - 1
                            annee = periode.year
                            lot = Model_LotBulletins.objects.filter(date_fin__month=mois,date_fin__year=annee,est_regulier=est_regulier).first()
                        ...
                        if int(periode.month) == 1: primepresence = primetotjour
                        elif int(periode.month) > 1: 
                            mois = periode.month - 1
                            annee = periode.year
                            lot = Model_LotBulletins.objects.filter(date_fin__month=mois,date_fin__year=annee,est_regulier=est_regulier).first()
                        ...
                        if int(periode.month) == 1: primepresence = primejrconge
                        elif int(periode.month) > 1: 
                            mois = periode.month - 1
                            annee = periode.year
                            lot = Model_LotBulletins.objects.filter(date_fin__month=mois,date_fin__year=annee,est_regulier=est_regulier).first()
                            item_bulletin = Model_ItemBulletin.objects.filter(bulletin__lot_id = lot.id, bulletin__employe_id = employe_id, rubrique_id = rubprimepresence.id).first()
                            primepresence =  makeFloat(item_bulletin.montant) + makeFloat(primejrconge)
